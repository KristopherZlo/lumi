...
- `RestoreService`: full restore, partial restore, pre-restore safety checkpoints, restore return points, operation orchestration, missing-baseline validation for `WORLD_ROOT`, and bounded mechanism target-state reconciliation; delegates request/variant safety checks to `RestoreRequestResolver`, `WORLD_ROOT` fallback baseline scoping to `WorldRootRestoreBaselineScope`, exact-position target reconstruction to `BlockTargetStateResolver`, restore chunk/position collection to `RestoreChunkCollector`, and post-apply metadata publication to `RestoreCompletionCoordinator`.
