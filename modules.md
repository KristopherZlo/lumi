...
- `RestoreService`: full restore, partial restore, pre-restore safety checkpoints, restore return points, operation orchestration, and bounded mechanism target-state reconciliation; delegates request/variant safety checks to `RestoreRequestResolver`, exact-position target reconstruction to `BlockTargetStateResolver`, and restore chunk/position collection to `RestoreChunkCollector`.
