...
| Restore, quick rollback, full rollback, operation progress | `RestoreService`, `QuickRollbackService` | `RestoreRequestResolver`, `RestorePlanBuilder`, `DirectRestorePatchPlanner`, `WorldRootRestoreBaselineScope`, `DirectRestorePatchPlan`, `RestorePayloadLoader`, `RestoreChunkCollector`, `BlockTargetStateResolver`, `RestoreEntityStateResolver`, `RestoreCompletionCoordinator`, `VersionLineageService`, `WorldOperationManager`, `ExactReplayStateQueue`, `ExactReplayStateGuard`, `FluidReplayUpdateScheduler`, `WorldOperationBossBarManager`, `WorldApplyBudgetPlanner`, `WorldChangeBatchPreparer`, `FluidSensitiveBlockReplayPolicy`, `PreparedWorldChangeBatches`, `MechanismReplayScope`, `SnapshotBatchPreparer`, `BlockChangeApplier`, `SectionContainerRewriteCommitStrategy`, `SectionNativeBlockCommitStrategy`, `DirectChunkBlockCommitStrategy`, `RecoveryRepository` | `RestoreServiceTest`, `DirectRestorePatchPlannerTest`, `QuickRollbackServiceTest`, `RestoreRequestResolverTest`, `RestorePlanBuilderTest`, `RestorePayloadLoaderTest`, `RecoveryRepositoryTest`, `WorldChangeBatchPreparerTest`, `WorldApplyBudgetPlannerTest`, `ExactReplayStateQueueTest`, `FluidReplayUpdateSchedulerTest`, `docs/architecture.md` |
...
- `RestoreService`: full restore, partial restore, pre-restore safety checkpoints, restore return points, operation orchestration, missing-baseline validation for `WORLD_ROOT`, and bounded mechanism target-state reconciliation; delegates request/variant safety checks to `RestoreRequestResolver`, direct patch replay planning to `DirectRestorePatchPlanner`, `WORLD_ROOT` fallback baseline scoping to `WorldRootRestoreBaselineScope`, exact-position block target reconstruction to `BlockTargetStateResolver`, entity target reconstruction and authoritative entity replacement batches to `RestoreEntityStateResolver`, restore chunk/position collection to `RestoreChunkCollector`, and post-apply metadata publication to `RestoreCompletionCoordinator`.
