...
- `ChunkSectionUpdateBroadcaster`, `WorldLightUpdateQueue`, `WorldLightUpdateContext`: batched section/block-entity client update packets and operation-scoped deferred light preparation, dirty-chunk preload, check drains, loaded dirty chunk marking with missing-chunk diagnostics, threaded light-engine barriers, and automatic `light-refresh` follow-up actions after block/entity replay.
