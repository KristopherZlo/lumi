...
| Restore, quick rollback, full rollback, operation progress | `RestoreService`, `QuickRollbackService` | `RestoreRequestResolver`, `RestorePlanBuilder`, `DirectRestorePatchPlanner`, `ExactRootStateRestorePlanner`, `RootLikeRestoreChunkResolver`, `RestoreUndoActionFactory`, `WorldRootRestoreBaselineScope`, `DirectRestorePatchPlan`, `RestorePayloadLoader`, `RestoreChunkCollector`, `BlockTargetStateResolver`, `RestoreEntityStateResolver`, `RestoreCompletionCoordinator`, `VersionLineageService`, `WorldOperationManager`, `WorldApplyTickDiagnostics`, `ExactReplayStateQueue`, `ExactReplayStateGuard`, `FluidReplayUpdateScheduler`, `WorldOperationBossBarManager`, `WorldApplyBudgetPlanner`, `WorldChangeBatchPreparer`, `FluidSensitiveBlockReplayPolicy`, `PreparedWorldChangeBatches`, `MechanismReplayScope`, `SnapshotBatchPreparer`, `BlockChangeApplier`, `SectionContainerRewriteCommitStrategy`, `SectionNativeBlockCommitStrategy`, `DirectChunkBlockCommitStrategy`, `RecoveryRepository` | `RestoreServiceTest`, `DirectRestorePatchPlannerTest`, `ExactRootStateRestorePlannerTest`, `RootLikeRestoreChunkResolverTest`, `RestoreUndoActionFactoryTest`, `QuickRollbackServiceTest`, `RestoreRequestResolverTest`, `RestorePlanBuilderTest`, `RestorePayloadLoaderTest`, `RecoveryRepositoryTest`, `WorldChangeBatchPreparerTest`, `WorldApplyBudgetPlannerTest`, `WorldApplyTickDiagnosticsTest`, `ExactReplayStateQueueTest`, `FluidReplayUpdateSchedulerTest`, `docs/architecture.md` |
| Partial restore | `RestoreService`, `PartialRestorePlanner`, `PartialRestoreTargetStatePlanner` | `PartialRestorePendingDraftProvider`, `SaveDetailsScreen`, `SaveDetailsScreenController`, `SaveDetailsPartialRestoreSection`, `LumiRegionSelectionController`, `LumiRegionSelectionTeachingController`, `LumiRegionSelectionRenderer`, `PatchDataRepository`, `SnapshotReader`, `BaselineChunkRepository` | `PartialRestorePlannerTest`, `PartialRestoreTargetStatePlannerTest`, `PartialRestoreFormStateTest`, `LumiRegionSelectionStateTest`, `SelectionToolTeachingStateTest`, `docs/storage-format.md` |
...
| Diagnostic telemetry, first-run notice, and backend ingest | `TelemetryService`, `TelemetryEventFactory`, `TelemetryEndpointPolicy`, `TelemetrySettingsRepository`, `TelemetrySpoolRepository`, `TelemetrySender`, `TelemetryEnvironmentProvider` | `TelemetryNoticeController`, `TelemetrySettingsPanelController`, telemetry section in `SettingsScreen`, client failure hooks in `LumaClient`, operation/rejection hooks in `WorldOperationTickRunner`, `WorldOperationShutdownHandler`, `QuickSaveScreenController`, `ProjectScreenController`, `RecoveryScreenController`, `ShareScreenController`, and overlay renderers, plus `telemetry-backend/src/server.js`, `telemetry-backend/src/service.js`, `telemetry-backend/src/validation.js`, `telemetry-backend/src/rate-limiter.js`, `telemetry-backend/src/pg-repository.js`, `telemetry-backend/src/retention-job.js`, and `telemetry-backend/src/retention-scheduler.js` | `src/test/java/io/github/luma/telemetry`, `src/test/java/io/github/luma/client/telemetry`, `src/test/java/io/github/luma/architecture/TelemetryIntegrationGuardrailsTest`, `telemetry-backend/test`, `README.md`, `docs/architecture.md`, `docs/development.md`, `docs/storage-format.md`, `docs/maintenance-guide.md` |
...
| Commands and runtime tests | `LumaCommands`, `LumaClientCommands`, `LumiClientGameTests`, `src/gametest/java/io/github/luma/minecraft/testing/*`, shared helpers in `src/runtimeGametestSupport/java/io/github/luma/minecraft/testing/*` | `ClientWorkspaceOpenService`, `SingleplayerGameplayRegressionSuite`, `SingleplayerBulkApplyDiagnostics`, `LumiBackupStressClientScenario`, `LumiTestFailpoints`, scripts under `scripts/` | Runtime regression suites run through `runClientGameTest` and `runHistoryJourneyClientGameTest`, not player commands; `LUMI_SINGLEPLAYER_TEST_MODE=backup-stress`, `docs/commands.md`, `docs/test-client.md` |
...
- `telemetry-backend/src/server.js`: minimal Node ingest API entry point for diagnostic telemetry, with bounded body reads, request validation, rate limiting, retention scheduling, and Postgres storage helpers in the sibling `src/` files.
...
- `VersionService`: save/amend versions, isolate operation drafts, write patch-first history, publish staged merge/save versions after successful apply, request snapshots/previews; delegates snapshot policy and chunk collection to `VersionSnapshotPlanner`.
- `RestoreService`: full restore, partial restore, pre-restore safety checkpoints, restore return points, operation orchestration, missing-baseline validation for `WORLD_ROOT`, and bounded mechanism target-state reconciliation; delegates request/variant safety checks to `RestoreRequestResolver`, direct patch replay planning to `DirectRestorePatchPlanner`, exact initial/`WORLD_ROOT` append planning to `ExactRootStateRestorePlanner`, root-like restore summary chunk selection to `RootLikeRestoreChunkResolver`, restore undo action construction to `RestoreUndoActionFactory`, `WORLD_ROOT` fallback baseline scoping to `WorldRootRestoreBaselineScope`, exact-position block target reconstruction to `BlockTargetStateResolver`, entity target reconstruction and authoritative entity replacement batches to `RestoreEntityStateResolver`, restore chunk/position collection to `RestoreChunkCollector`, and post-apply metadata publication to `RestoreCompletionCoordinator`.
- `RestoreCompletionCoordinator`: completes full and partial restore after prepared world apply succeeds, including pending completion records, full-restore variant-head/project metadata updates, partial-restore draft replacement, undo/redo cleanup or action recording, journal entries, and capture-cache invalidation.
...
- `PartialRestoreTargetStatePlanner`: selected-area and outside-selection target-state reconstruction for partial restore, including whole-dimension tracked-chunk scoping for redstone/mechanism restores.
...
- `CaptureBaselineCoordinator`, `SessionBaselineStateResolver`: active-session baseline snapshots, pre-mutation baseline corrections, and same-session direct mutation rebasing so transient local states do not become the recovery-draft old value.
...
- `ExplosiveEntityContextRegistry`: TNT/explosion causal context, including redstone-primed TNT and TNT-chain action ownership through delayed fuse and `TntBlock.wasExploded` ignition paths.
...
- `WorldOperationManager`, `WorldOperationLifecycle`, `WorldOperationTickRunner`, `WorldOperationShutdownHandler`, `WorldOperationMetricsReporter`, `LightRefreshActiveOperation`, `WorldOperationRegistry`, `WorldApplyTickDiagnostics`, `WorldApplyBudgetPlanner`, `WorldApplyOperationProfile`, `WorldApplyPerformanceGovernor`, `ApplyCostModel`, `WorldApplyChunkPreloader`, `ServerLevelChunkPreloadAccess`, `WorldApplyChunkLoadContext`, `WorldApplyChunkResolver`, `WorldApplyNoOpPruner`, `WorldApplyVerificationService`, `WorldApplyVerificationRepairer`: single-operation-per-world async prepare plus optional fast-profile chunk preload and tick-time apply orchestration, including explicit normal/history-fast/diagnostic/maximum profiles for preload/sync-load/sparse-block/native-cell/rewrite/redstone/light/block-entity/entity budgets, stall-aware one-at-a-time sync preload recovery after ticket progress stops, active/last operation state tracking with a bounded recent-operation window, per-tick advance failure handling, shutdown drain/failure handling, final metrics summary reporting, per-tick apply detail/shape diagnostics, light-refresh drain/barrier/publish follow-up handling, observed kind-specific work cost gating, fast-profile-only chunk reacquire after preload across direct/native/rewrite paths, fast-profile mixed rewrite/native/sparse work within one tick budget, loaded-chunk no-op pruning that preserves only real updates plus forced/adjacent/boundary exact replay and promotes dry targets over live fluid into fluid replay cleanup hints, touched-position post-apply verification/repair for restore-style operations, and minimum fast-profile sparse/direct floors after adaptive downscale, with final apply metrics available to runtime tests.
- `WorldChangeBatchPreparer`, `FluidSensitiveBlockReplayPolicy`, `PreparedWorldChangeBatches`, `MechanismReplayScope`, `EntityApplyMode`, `RestoreEntityCleanupPolicy`, `BlockStatePaletteDecoder`: patch/recovery block/entity changes, large live undo/redo actions, v9 section frames, and exact target-state maps to tick-ready sparse or section-native batches with operation-scoped palette decode caching, dry water-displaced block replay classification, mechanism replay scope analysis, and explicit delta versus authoritative entity replacement modes.
...
- `WorldApplyTickDiagnostics`: per-apply-tick counter deltas, block-apply detail formatting, chunk set/delete/native/rewrite/sparse shape counting, and commit-result summaries used by debug/load/block-apply logs.
...
- `MechanismStatePolicy`, `ExactReplayStateGuard`, `ExactReplayStateQueue`, `ExactReplayGuardBlockPolicy`, `FluidReplayUpdateScheduler`, `WorldReplayTickSuppression`, `DeferredActionFalloutGuard`: shared redstone/mechanism classification plus short stale-callback replay protection for derived states, replay-restored TNT, and replay-created mechanism callbacks. Fluid replay hints keep stored target cells under the exact-state guard while loaded connected fluid tails receive bounded vanilla ticks, and replay-to-air or live-fluid-to-dry targets immediately clear loaded non-source fluid in the target cells plus bounded orphaned non-source components only when the component is fully inspected and separated from source/saved fluid by the restored dry targets. Source-connected older/generated water, unknown unloaded tails, and saved source water/lava are preserved. Mechanism replay hints keep target-air mechanism removals in final replay and suppress stale callback envelopes. Piston bases, piston heads, moving pistons, and observers are not held by the exact-state guard unless an explicit mechanism replay hint targets air; their replay callback envelope is suppressed briefly instead. Mechanism replay and callback decisions are logged through `HistoryDebugLog` when debug tracing is enabled.
...
- HUD, selection, and compare/pending/recent overlays: `WorkspaceHudCoordinator`, `LumiRegionSelectionController`, `LumiRegionSelectionTeachingController`, `LoadedChunkBlockRaycaster`, `LumiRegionSelectionRenderer`, `CompareOverlayPreparationService`, `CompareOverlayCoordinator`, `CompareOverlayRenderer`, `CompareOverlayHotkeyHud`, `CompareOverlaySurfaceResolver`, `CompareOverlayRenderTypes`, `RoundedHudRenderer`, `PendingChangesOverlayCoordinator`, `PendingChangesOverlayRenderer`, `RecentChangesOverlayCoordinator`, `RecentChangesOverlayRenderer`, `OverlayMeshBatch` for cached section GPU meshes, `OverlayVolumeMerger` for bounded tiled over-cap compare and pending blobs, `OverlayFaceRenderer`, `OverlayDiagnostics`, `ClientRuntimeLoadSampler` for test-client frame/load sampling. Current-world compare refreshes are throttled and reuse existing meshes when the diff content is unchanged.
...
- Fabric runtime suites: `src/gametest`, including `LumiScreenClientGameTests` for live screen render/button smoke coverage, shared non-mod helpers in `src/runtimeGametestSupport`, plus `src/historyJourneyGametest`, `src/idleGametest`, `src/baselineGametest`, and `src/baselineIdleGametest`.
