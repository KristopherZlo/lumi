...
- `RestoreService`: full restore, partial restore, pre-restore safety checkpoints, restore return points, operation orchestration, missing-baseline validation for `WORLD_ROOT`, and bounded mechanism target-state reconciliation; delegates request/variant safety checks to `RestoreRequestResolver`, `WORLD_ROOT` fallback baseline scoping to `WorldRootRestoreBaselineScope`, exact-position block target reconstruction to `BlockTargetStateResolver`, entity target reconstruction and authoritative entity replacement batches to `RestoreEntityStateResolver`, restore chunk/position collection to `RestoreChunkCollector`, and post-apply metadata publication to `RestoreCompletionCoordinator`.
