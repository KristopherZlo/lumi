...
- `WorldOperationManager`, `WorldApplyBudgetPlanner`, `WorldApplyOperationProfile`: single-operation-per-world async prepare plus tick-time apply orchestration, including explicit normal/history-fast/diagnostic profiles for sparse-block/native-cell/rewrite/light budgets, with final apply metrics available to runtime tests.
...
- `SectionContainerRewriteCommitStrategy`, `SectionRewriteApplyPlanner`, `PalettedContainerDataSwapper`, `SectionNativeBlockCommitStrategy`, `DirectChunkBlockCommitStrategy`, `ChunkHeightmapUpdatePlan`, `HeightmapColumnUpdater`, `DirectSectionBlockCommitStrategy`, `VanillaBlockCommitStrategy`: dense section container rewrite path, section-native loop path, chunk-level direct sparse apply path with per-column heightmap coalescing, and safe vanilla fallback selection.
