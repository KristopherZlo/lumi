...
- live undo and redo for the last tracked builder actions with default `Left Alt+Z` / `Left Alt+Y` bindings through the remappable Lumi action button; changing the action button changes these chords too. WorldEdit, FAWE, and captured Axiom actions route those chords through the tools' native undo/redo surfaces first, with Lumi replay as the fallback.
...
6. Use the Lumi action button plus `Z` / `Y` to undo or redo the latest tracked action while no screen is open. The default action button is `Left Alt`, and changing it changes these chords too. WorldEdit/FAWE/Axiom actions use native tool undo/redo first, with Lumi replay as the fallback.
