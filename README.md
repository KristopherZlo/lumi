...
- Recovery drafts are serialized once per flush; save detaches and reconciles the final live draft on the server thread, then waits for its durable flush on the operation worker before isolating the operation draft. Zone save reuses that durable draft instead of rewriting it again, and the operational journal retains only its newest 512 entries. Recovery base/WAL reads, appends, compaction, and deletion share one project-path lock, so an async append cannot be mistaken for a truncated crash tail. Project dirty block sections and entity chunks are coalesced in the compact `recovery/dirty-scope.bin` sidecar instead of storing per-tick payloads.
...
- In integrated singleplayer, an otherwise unowned creeper explosion or lightning strike opens one non-player `world incident` action. Its bounded fire, scheduled-tick, block, and supported entity fallout share one action id for live undo and dirty-scope protection; later spread is excluded after that action context expires.
