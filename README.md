...
- Live undo may track transient entities to clean up active fallout, including killed mobs from player-caused explosion and mob actions. Causal mob deaths use the pre-damage or pre-interaction entity snapshot and drain through a deduplicated batched tick queue, so restored entities do not replay death or ignition state; recovery drafts and saved commits must not persist undo-only transient entities.
