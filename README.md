...
- Existing workspaces mark persistent block and supported block-entity mutations dirty before causal attribution policy runs. Actionless redstone, fluid, fire, growth, piston, and other vanilla block fallout therefore enters the recovery draft and saved history through coalesced section stabilization, so quick rollback can restore it without scanning untouched chunks; it still does not create a live undo action or bootstrap a new workspace.
