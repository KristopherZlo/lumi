...
- non-player entity edits from explicit player and supported builder-tool actions are saved with their full captured NBT position and state, including mobs, item entities, display entities, paintings, and armor stands
...
- undo/redo replays stored block states with side-effect-suppressed placement flags; redstone power/source transitions are deduplicated and replayed as a bounded final neighbor-update phase after all stored blocks have been written, while ordinary replay still avoids placement physics and suppresses piston extension checks during internal apply so saved moved-block deltas are not replayed twice
