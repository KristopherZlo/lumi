...
6. Whole-dimension sessions keep a causal chunk envelope rooted in explicit builder edits. The root chunk defines a one-chunk halo envelope, and Lumi captures per-chunk baselines lazily when a chunk inside that envelope first needs stabilization. Deferred piston, redstone, fluid, and falling-block mutations inside an active session region can also seed that session baseline before their first block write, without bootstrapping new ambient sessions.
7. Ambient fallout such as fluid spread and falling blocks no longer appends directly into the working draft. It only re-marks chunks inside that causal envelope as dirty, and water-triggered neighbor fallout remains under the fluid source. Piston reconciliation records both new settled deltas and tracked moved cells that returned to the baseline. Moving piston block entities preserve the original action id across piston-carrier chains, while redstone block events and scheduled ticks still use bounded causal context so closed observer/piston loops cannot keep recording periodic ticks as new builder history.
8. Related item drops from player-caused entity deaths, explosions, water flow, falling blocks, and block-update fallout are attached to the latest matching undo/redo action only; secondary-only drops are deliberately excluded from recovery drafts and saved versions.
9. `TrackedChangeBuffer` merges explicit and targeted realtime block changes by position and entity changes by UUID immediately as the durable working draft; `LiveUndoRedoActionRecorder` mirrors eligible action-scoped changes into the separate in-memory undo/redo stack.
