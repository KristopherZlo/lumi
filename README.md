...
- If a crash lands after the branch head is published but before dirty-scope cleanup, startup retains every dirty section, rebases the ledger to the published head, and discards only the now-stale isolated operation draft covered by that ledger. The next reconciliation consumes already-saved no-op sections without publishing an empty version while preserving later live differences.
