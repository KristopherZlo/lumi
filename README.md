...
- redstone and mechanism state is saved and replayed as final settled block state, including lever `powered`, wire `power`, lamp `lit`, openable `open`, repeater/comparator state, piston base `extended`, settled `piston_head`, and moved blocks. Apply preparation completes missing settled piston head/removal companions from an explicit piston base, but never creates a piston base from a head-only record. Only short-lived `moving_piston` animation state is normalized away
