...
- Builder-facing Build History UI with save details, branches, compare, restore, import/export, recovery, cleanup, settings, a first-level history graph, and deleted-save views.
...
- Ambient changes without a causal builder action are rejected. Causal secondary fallout, including bonemeal growth, can be stored hidden so restore/undo/redo can replay it without adding builder-facing noise.
...
- whole-dimension root fallback uses tracked baseline chunks only when direct replay is not valid; direct root rollback reads exact baseline state only for replayed block positions plus bounded redstone/mechanism reconciliation scope
...
- World apply, exact replay, and post-replay guard paths treat recoverable `Exception`/`RuntimeException` failures as operation data, not client-fatal errors. Corrupt block-entity/entity tail payloads are skipped with bounded summaries, exact replay targets are quarantined after repeated failures, and operation metrics include `applyFailures`.
