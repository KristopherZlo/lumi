...
- undo/redo replays stored block states with side-effect-suppressed placement flags; durable redstone/mechanism state transitions send scoped neighbor updates so toggled controls settle connected circuitry, while ordinary replay still avoids placement physics and suppresses piston extension checks during internal apply so saved moved-block deltas are not replayed twice
