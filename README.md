...
- Same-Minecraft-version update checks at client startup, with a clickable in-world chat notice after world entry and a Build History update card backed by the Lumi website plus GitHub raw manifest fallback.
...
Update checks run on client startup and are reused by the world-entry chat notice and Build History update card. They can be overridden for local testing with:
