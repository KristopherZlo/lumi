...
- runtime-only redstone state flips and piston animation blocks are deferred through session stabilization and then ignored when they do not produce structural changes, so active mechanisms do not pollute pending history or the recent action overlay
...
11. Before draft snapshots, idle flushes, save, amend, undo/redo selection, or freeze persist or consume anything, Lumi reconciles dirty envelope chunks on the server thread against the current world and stores the final stabilized diff on top of the live pending chunk buffer. Property-only redstone deltas such as `lit`, `powered`, and `power` are filtered during that composition, so storage remains schema-compatible state history rather than tick-by-tick redstone event history.
