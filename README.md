...
7. On first entry to an existing world, Lumi creates a conservative pre-mod backup under `lumi/pre-mod-backup`: generated-pristine region chunks are skipped, while chunks with persisted player-activity markers are stored as maximum-compression raw NBT.
