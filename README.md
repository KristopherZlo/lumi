...
- Existing workspaces mark every confirmed persistent block mutation, including actionless ambient fallout, in the project dirty-section sidecar after capturing any missing pre-mutation chunk baseline. Action attribution still controls player undo, ambient fallout still cannot create a workspace, and internal Lumi apply plus unowned generation/deserialization sections are excluded. Axiom buffer edits preflight baselines and bulk-mark unique sections instead of creating dirty payloads per block.
