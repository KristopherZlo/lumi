...
- Use `/animove <radii> <x> <y> <z> [x] [y] [z] [animal_selector] [-loop]` to make nearby animals run to a point, optionally return to a second point, and optionally loop until `/animovestop [animal_selector]`.
- Use `/corrupt on` to plan a stable world-space simplex-noise density mask projected onto the visible terrain surface, apply those ground changes in batches with restorable missing-texture blocks that leak random light levels, spawn upward ash from the ground, spawn slow upward-floating non-glowing mobs/entities and 3x3/5x5 rising terrain slices, offset the current time of day by -2000 to +2000 ticks with a visible particle flash every 1-3 seconds, and spawn jittering display-only `block_display` glitches in the sky with both missing-texture and normal block states; `/corrupt off` removes sky glitches and restores the captured original block states behind a white healing wave expanding from the player, with a client-side shader fade over restored block surfaces.
- Press `K` or run `/corruptsettings` to open the live corruption settings menu. It does not pause the game, slider changes apply immediately, and the menu exposes the current world-space mask, ash, sky display, `/corrupt off` cleanup delay, cleanup spread speed, white fade duration, and optional blackout healing mode settings.
