...
- Use `/corrupt on` to batch-replace nearby non-air blocks with a missing-texture corruption block, spawn basalt-delta-style ash particles, and spawn jittering `block_display` glitches in the sky; `/corrupt off` removes sky glitches and queues restoration of the captured original block states.
