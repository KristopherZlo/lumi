...
- Press `K` or run `/corruptsettings` to open the live corruption settings menu. It does not pause the game, slider changes apply immediately, and the menu exposes the current world-space mask, ash, sky display, and `/corrupt off` cleanup delay settings.
