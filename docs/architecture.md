...
- `RecentChangesOverlayRenderer` renders latest undo actions when the remappable Lumi action button is held, or redo actions while the action button plus redo is held, when the compare overlay is not active. Dense action previews are selected from exposed changed blocks first, capped to the nearest 512 per-block entries, and the omitted dense volume is summarized by low-alpha chunk-section aggregate boxes so large edits remain readable without drawing thousands of overlapping overlays.
