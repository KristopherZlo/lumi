...
- `WorldOperationManager`: runs async preparation plus completed-first chunk-queue dispatch on the server tick with explicit normal/history-fast/diagnostic apply profiles, adaptive block budgets, bounded section-rewrite and sparse direct-section passes, and bounded block-entity/entity tail work
