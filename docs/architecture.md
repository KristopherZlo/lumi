...
- redstone restore is state-based, not event-based: prepared apply writes the selected persistent block states with side-effect-suppressed flags and does not fire immediate neighbor-update or piston replay cascades. Settled piston replay is base-driven: an extended base can add its expected `piston_head`, a retracting base can clear the old head position, and a head-only placement is never used to create a new adjacent piston base
