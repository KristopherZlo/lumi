...
- `ChunkSectionUpdateBroadcaster`, `WorldApplyBlockUpdatePolicy`, `RedstoneReplayUpdatePlanner`, and `BlockChangeApplier`: commit section blocks, block entities, and entity batches in bounded steps with batched section packets, block-entity packets, and side-effect-suppressed fallback flags. Durable redstone/mechanism state transitions emit scoped neighbor updates after commit so toggled controls settle connected circuitry, while ordinary replay still avoids placement physics and suppresses piston extension checks during internal apply so captured moved-block deltas are not replayed twice.
...
- redstone restore is state-based, not pulse-based: prepared apply writes the selected persistent block states with side-effect-suppressed flags, then emits scoped neighbor updates only for durable redstone/mechanism state transitions. Settled piston replay is base-driven: an extended base can add its expected `piston_head`, a retracting base can clear the old head position, normalized transient air at the expected head can be replaced by the settled head, and a head-only placement is never used to create a new adjacent piston base
