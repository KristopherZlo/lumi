...
- `PendingChangesOverlayCoordinator` prepares the cumulative unsaved recovery draft overlay off the client tick while the remappable Lumi action button is held and no compare or recent-action overlay is visible. `PendingChangesOverlayRenderer` renders visible pending draft block changes since the active head, using cached exposed-surface meshes below the detailed cap and bounded tiled orange volume blobs above it. Hidden natural-growth changes remain in the draft but are filtered out before overlay geometry is built.
- `RecentChangesOverlayCoordinator` prepares recent-action overlay data off the client tick while the action button is held and pins the selected undo/redo stack revision for that hold so live edits do not flicker or replace an existing preview. `RecentChangesOverlayRenderer` renders undo and redo actions together when the compare overlay is not active. The first selected undo action renders red, the first selected redo action renders green, and older recent actions remain orange. Action previews resolve exposed changed block faces once, ignore hidden natural-growth changes, then render cached section-scoped GPU meshes with lazy uploads and render-distance culling at small and giant sizes instead of falling back to coarse merged volume chunks.
...
2. `WorldMutationContext` accepts explicit player/tool scopes and targeted secondary scopes. Ambient random ticks are marked as `GROWTH` and do not inherit player action IDs, so natural kelp, vine, grass, or amethyst growth cannot masquerade as a builder action. Captured `GROWTH` block deltas are stored with a hidden visibility bit: restore and recovery can replay them, but builder-facing stats, diffs, overlays, and preview bounds ignore them.
...
The current durable history format is project schema v4, patch payload schema v8, snapshot payload schema v5, and recovery draft schema v5.
