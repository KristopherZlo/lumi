...
- `CompareOverlayCoordinator` refreshes `current`-world compare overlays on the client tick so live edits appear in the active highlight without rebuilding the screen manually. To protect the client, active overlays above the same 50,000-block detailed-render cap keep their initial snapshot instead of auto-refreshing every few ticks.
- `RecentChangesOverlayCoordinator` prepares recent-action overlay data off the client tick and pins the selected undo/redo stack revision for the duration of a held overlay key so live edits do not flicker or replace an existing preview. `RecentChangesOverlayRenderer` renders latest undo actions when the remappable Lumi action button is held, or redo actions while the action button plus redo is held, when the compare overlay is not active. Action previews resolve exposed changed block faces once, then render cached section-scoped GPU meshes with lazy uploads and render-distance culling at small and giant sizes instead of falling back to coarse merged volume chunks.
