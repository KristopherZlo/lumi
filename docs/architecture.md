...
- `CommitGraphLayout` computes deterministic branch lanes, skips shared-only empty lanes, and exposes parent-lane connectors, while `CommitGraphComponent` renders the More history graph as colored lanes, branch-head badges, and commit metadata instead of ASCII prefixes.
