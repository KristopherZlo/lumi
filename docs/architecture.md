...
- `CompareOverlayRenderer` renders a client-side compare overlay with a remappable hold-to-x-ray mode, keeps diff data separate from visibility, binds active overlay data to the resolved project/version pair, prioritizes the nearest exposed changed blocks to the current camera position, and draws exposed overlay faces through immediate end-main quads so translucent fill is flushed independently of the shared world buffer. Very large diffs skip per-block surface resolving and collapse changed chunk sections into low-alpha merged volume blobs by change type. Normal mode remains depth-tested so highlights do not show through blocks; x-ray mode deliberately disables depth testing.
