...
- bounded chunk preload and chunk-batch application on the server tick through `WorldOperationManager`, including adaptive block budgets, explicit preload ticket caps, no default synchronous chunk-load fallback during preload, `WorldApplyPerformanceGovernor` decisions from `ApplyCostModel`, fast-profile-only synchronous chunk reacquire after preload misses, fast-profile mixed rewrite/native/sparse tick work, profile-specific minimum sparse/direct time floors after adaptive downscale, and explicit block-entity/entity caps
