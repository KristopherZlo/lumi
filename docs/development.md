...
- Automatic dimension project bootstrap is limited to explicit builder-driven sources. Ambient fluid, fire, growth, block-update, and mob mutations cannot create a workspace on world load by themselves. Captured natural growth is stored with hidden block-change visibility so it remains replayable but does not affect builder-facing summaries, overlays, or preview framing.
...
- debug apply metrics include rewrite sections/cells, native sections/cells, preload pipeline counters, deferred redstone updates, deferred light checks, light-engine flush ticks, section packets, and rewrite/native fallback reasons so slow dense actions can be traced to safety rejection, redstone or light maintenance, block entities, unloaded chunks, or sparse distribution.
