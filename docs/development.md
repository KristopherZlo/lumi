...
- pressing the Lumi action button plus `Y` starts redo for the latest tracked action in the current dimension workspace while no client screen is open; if undo and redo are pressed in the same tick, undo wins and redo must be pressed again. WorldEdit and FAWE actors route through native `/undo` and `/redo`, with Lumi capture suppressed during the command and the pending draft adjusted afterward; Axiom actors stay in Lumi replay so stack-detected Axiom block changes, block-buffer edits, and Axiom-assisted break actions can be undone and redone by Lumi.
