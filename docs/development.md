...
- compare, recent-action, and region-selection overlay geometry is cached into section-scoped GPU meshes. Overlay states rebuild CPU geometry only when the diff, held recent-action revision, or selected bounds change; frame rendering reuses uploaded buffers and lazily uploads newly visible sections instead of rebuilding all vertices every frame. Recent-action overlays keep exposed changed-block surface meshes even at giant action sizes, and the selected Alt preview is pinned while the key is held so live edits do not collapse or replace it.
