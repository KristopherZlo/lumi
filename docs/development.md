...
- prepared world operations log fast-apply metrics under `world-op` debug tracing, including direct/fallback sections, section packets, block-entity packets, light checks, and fallback reasons
...
- pressing the Lumi action button plus `Y` starts redo for the latest tracked action in the current dimension workspace while no client screen is open; if undo and redo are pressed in the same tick, undo wins and redo must be pressed again. WorldEdit and FAWE actors route through native `/undo` and `/redo`, with Lumi capture suppressed during the command and the pending draft adjusted afterward; Axiom actors or `axiom-*` action ids route through Axiom's native undo/redo hook first, suppressing Lumi capture while the delegated block-buffer replay runs, and fall back to Lumi replay when that hook is unavailable or cannot move Axiom history.
...
- direct loaded-section block commits for prepared apply batches, with vanilla fallback and batched section/client block-entity updates
