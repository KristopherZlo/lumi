...
- compare, pending-draft, recent-action, and region-selection overlay geometry is cached into section-scoped GPU meshes. Overlay states rebuild CPU geometry only when the diff, pending draft revision, held recent-action revision, or selected bounds change; frame rendering reuses uploaded buffers and lazily uploads newly visible sections instead of rebuilding all vertices every frame. Plain Lumi action hold shows cumulative unsaved draft changes, while action+undo/redo shows the pinned recent action preview.
...
- opening See Changes with a resolved pair or pressing `Compare` starts background diff loading; the world highlight enables once that diff is ready
...
- compare overlays and recent-action previews build their cached section meshes from exposed changed blocks, so dense fills still have visible surfaces even when the camera is nearest to internal changed blocks; compare overlays above 50,000 changed blocks silently collapse to merged low-alpha volume blobs by change type
