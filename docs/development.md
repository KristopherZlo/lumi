...
- Optional external builder tools use explicit mutation sources. WorldEdit sessions are observed through a guarded `EditSessionEvent` extent wrapper when WorldEdit is present; the wrapper records old/new block transitions directly because some WorldEdit bulk paths do not surface through Minecraft `Level#setBlock`. Axiom does not expose a stable operation API, so Lumi detects Axiom-origin server mutations from external stack frames at the `LevelChunk#setBlockState` fallback and records those transitions as `WorldMutationSource.AXIOM`.
