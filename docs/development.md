...
- undo-only item drops and primed TNT entities from direct ignition or redstone activation attached to live undo/redo without persisting into recovery drafts or saved versions
...
- branch-scoped history package export/import, imported review package deletion, cached imported-branch merge planning, incoming-over-current merge projection, and imported payload safety warnings
...
- architecture guardrails that keep Java stack sampling out of mixin/capture/world tick hot paths and keep storage decoders out of world-apply tick code
- lightweight home, branches, and share controller loading plus operation-state normalization
- fast world-apply section packet masks, direct-section eligibility fallback reasons, and apply metric summaries
- mechanism-aware restore/rollback reconciliation, including shared redstone classification, mechanism replay hints, direct restore fallback on large scopes, quick rollback selection clipping, and partial restore target-state fallback for mechanism payloads
...
Run the integrated-world runtime regression suite through the client GameTest profile:

```powershell
.\scripts\run-test-client.ps1 -GradleTasks runClientGameTest
```

`runClientGameTest` enables `-Dlumi.testing.enabled=true` and starts the runtime harness from `src/gametest`, not from a player command. Set `LUMI_SINGLEPLAYER_TEST_MODE=full`, `player-flow`, `structure-fixtures`, `external-tools`, `crash-safety`, or `backup-stress` before the task to select a focused or destructive mode. The smoke path creates an archived temporary bounded project and drives bootstrap storage, snapshot content refs, patch reads, save/amend, undo/redo, branch/export, partial restore, full restore, integrity, cleanup, and the final smoke screenshot through real services. `full` adds gameplay, performance, large-history, bulk apply, TNT/fluid/entity, and structure-fixture diagnostics; `player-flow` runs from ordinary terrain; `backup-stress` keeps the pre-open backup stress path under `-Xmx1G`.

Runtime GameTest helpers that are shared by the normal runtime suite and the history journey suite live in `src/runtimeGametestSupport`. That source set has no Fabric mod metadata and keeps fixture support out of production `src/main` classes.

The client GameTest launchers still mute the run directory, enable v-sync, and cap `maxFps` at `120`. Screen and overlay smoke GameTests remain separate from the runtime harness, and `runBaselineClientGameTest` keeps the no-Lumi comparison path.
...
- client runtime load log: `-Dlumi.clientLoadLog=true`, writing `logs/lumi-client-load.log` under the game run directory by default. Test-client and client GameTest profiles enable it automatically. It records CPU, JVM memory, GC, direct/mapped buffer memory, render-frame pressure, OpenGL renderer info, and optional `nvidia-smi` GPU utilization/memory samples.
...
- restore/rollback block apply diagnostics: `-Dlumi.blockApplyLog=true`, writing `logs/lumi-block-apply.log` by default. This is also enabled when `lumi.loadLog` is enabled. It records restore/undo/redo/quick rollback preparation, preload ticks, chunk-level set/delete target counts, native/rewrite/direct apply steps, block-entity/entity steps, touched-position verification matched/mismatched/repaired/skipped counts, tick stop reasons, fallback summaries, and aggregate timings without logging every block.
- selected partial restore diagnostics: `-Dlumi.partialRestoreLog=true`, writing `logs/lumi-partial-restore.log` by default. This is not enabled by the broad load log. The `scripts/run-test-client.ps1` wrapper accepts this flag after its own parameters and also exposes `-JvmArgs` for multiple flags. It records `selected-current` live samples from the selected bounds before queuing the operation, `planned-target` old-to-new target changes with out-of-bounds counts, and `post-apply-remaining` mismatches read on the server thread after apply but before metadata completion.
- selected-area restores with redstone or mechanism changes use target-state planning instead of bounded patch replay. In whole-dimension projects, target-state planning ignores selected chunks that Lumi has never tracked in a baseline, snapshot, patch, or pending draft; if a tracked mechanism restore still lacks authoritative target state, the restore aborts rather than applying a partial patch replay.
...
- accepted capture sessions keep the first 32 per-mutation traces behind debug logging, while info level stays focused on buffer checkpoints, queued/completed maintenance work, and reconcile summaries. `CaptureDiagnosticsLogger` owns those capture traces, progress checkpoints, reconcile summaries, and throttled skipped-capture logs.
- whole-dimension stabilization now logs dirty-chunk reconcile summaries before draft flush/save/freeze, so startup diagnostics and reconcile summaries should be inspected together when ambient fallout looks suspicious. Delayed redstone and piston carriers restore their copied action context before they mutate the world, and stabilization writes their final settled chunk deltas into that same live undo/redo action when the action id is still authorized and the dirty chunk has stayed unchanged for the short tick-settle window. TNT primed by redstone or another TNT explosion keeps the originating builder action through the delayed fuse, including `TntBlock.wasExploded` chain ignition, so one Alt+Z owns the full TNT chain instead of splitting follow-up blasts into separate or actionless rollback entries. If that chunk still contains `moving_piston`, reconciliation requeues it instead of normalizing the animation cell into final air. Deferred piston, redstone, fluid, and falling-block sources can seed the session chunk baseline and pre-change cell corrections before their first block write when they are already inside an active session region and still carry or reuse a causal action id. Fluid-driven neighbor callbacks remain fluid fallout so water-broken blocks undo with the water action. Live undo/redo rebases secondary block fallout against the latest applied stack state at each position, so a later water release removes water from older dry cuts without restoring those cut blocks, while water-broken blocks from earlier explicit actions still return. That delayed mechanism context is propagation-bounded, and secondary physics/mechanism stabilization without a causal action id is skipped, so closed clocks, passive cave fluids, falling blocks, and random ticks cannot keep appending world-settling noise to the working draft. During undo/redo replay, stale delayed piston callbacks with the replayed action id are suppressed instead of being held through exact-state guards on piston or observer cells. For piston, observer, moved-block, and live undo/redo diagnostics, enable debug tracing and inspect `history-action`, `capture-block`, `mechanism-replay`, and `mechanism-callback`; callback-level traces are global mechanism hooks and require `-Dlumi.debug=true`.
- prepared world operations keep their final fast-apply metrics available to runtime tests and also log them under `world-op` debug tracing, including prepare/preload/apply/light/total durations, max apply/preload tick times, native sections/cells, rewrite sections/cells, direct/fallback sections, section packets, block-entity packets, touched-position verification counters, deferred light checks, apply ticks, work-per-tick counters, light-drain ticks/duration, and fallback reasons. `WorldOperationLifecycle` owns active/recent operation registration, `WorldOperationTickRunner` owns per-tick advance budgeting/failure handling, `WorldOperationShutdownHandler` owns server-stop drain/failure handling, `WorldOperationMetricsReporter` owns final duration/summary reporting, `WorldApplyTickDiagnostics` owns per-tick apply details, chunk shape counting, and commit-result summaries, and `LightRefreshActiveOperation` owns deferred light drain, barrier, publish, and inherited ticket release, so `WorldOperationManager` stays focused on queueing and active-operation lookup. The `world-op-apply` debug category adds per-apply-tick budgets, stop reasons, chunk batch composition, section/chunk path timings, fallback summaries, loaded-chunk no-op prune ratios, predicted chunk cost, tick pressure, verification repair counts, and deferred light drain counts for restore/undo/redo speed diagnosis. The separate `lumi-block-apply.log` and `lumi-light.log` files are the preferred first stop for large restore bottlenecks and rejoin-only shadows because they isolate block mutation timing from light refresh timing. Apply uses explicit `NORMAL`, `HISTORY_FAST`, `DIAGNOSTIC_TURBO`, and `MAXIMUM` profiles: ordinary work keeps conservative limits, restore/recovery/merge/undo/redo/light-refresh use the foreground `MAXIMUM` profile, bulk diagnostics keep larger sparse direct-section and light-drain caps, and high-throughput operations adapt down against the responsive server-tick pressure window while kind-specific cost modelling avoids starting another chunk when observed sparse/native/rewrite/block-entity/entity cost predicts an overrun. Ticketed preload avoids default synchronous chunk loads and switches to one-at-a-time synchronous recovery only after all tickets are issued and no ticket progress is observed.
...
- `src/main/java/io/github/luma/domain`
  Product-facing models and services for projects, versions, branches, recovery, diff, preview, and integrity.
- `src/main/java/io/github/luma/storage`
  Save-file layout plus repositories for metadata, patches, snapshots, branches, and recovery.
...
Application UI icons are stored in `src/main/resources/assets/lumi/textures/gui/icons/`. Every icon is a 16x16 px PNG with a matching `_disabled` 16x16 px PNG so the enabled and disabled states can be redrawn by hand as pixel-friendly assets.

| Icon name | Purpose | Resolution |
| --- | --- | --- |
| `chevron-down` | Expand More/details sections, currently in compare details. | 16x16 px |
| `chevron-up` | Collapse More/details sections, currently in compare details. | 16x16 px |
| `eye` | Open or show compare/highlight actions from branches, save details, and compare controls. | 16x16 px |
| `eye-off` | Hide the active compare/highlight view. | 16x16 px |
| `folder-open` | Open saves, save details, imported projects, or project history from compact cards. | 16x16 px |
| `git-branch` | Branch-related actions, including creating a branch from a save and comparing with a parent save. | 16x16 px |
| `rotate-ccw` | Restore a selected save from a history card. | 16x16 px |
| `trash-2` | Delete a branch, save, or imported package. | 16x16 px |
| `zoom-in` | Zoom in on a save preview. | 16x16 px |
| `zoom-out` | Zoom out on a save preview. | 16x16 px |

The icon folder also contains `info`, which is reserved for information/help actions but is not currently referenced by an icon button.
...
- undo-only item drops from explosion, fluid, falling-block, and related block-update fallout plus primed TNT entities from ignition or redstone activation are removed on undo and respawned on redo, while durable drafts and saved versions keep only the block/entity history they should persist
- undo/redo applies the selected stored states with client-visible but side-effect-suppressed block update flags. Redstone power/source transitions then drain scoped neighbor updates after the stored state has been written, so lever and button rollback settles adjacent circuitry without enabling broad placement physics or piston event cascades during replay. The bounded exact replay guard holds derived volatile redstone/mechanism states and replay-restored TNT, not player input controls, and it is released when a new explicit player/tool mutation begins.
...
- comparing against `current` refreshes the active world highlight automatically on the normal client cadence while that highlight is visible, except active overlays above the 50,000-block detailed-render cap which keep their initial snapshot to avoid client stalls. If the refreshed diff is unchanged, the existing overlay meshes are reused. The compact compare hotkey HUD is tied to overlay visibility, so pressing `H` hides the world highlight and its hint panel together.
...
- compare overlays and recent-action previews build their cached section meshes from exposed changed blocks, so dense fills still have visible surfaces even when the camera is nearest to internal changed blocks; compare overlays above 50,000 changed blocks silently collapse to bounded tiled low-alpha volume blobs by change type and render through blocks without requiring the action-button x-ray hold. `BuilderChangeSurfacePolicy` owns the shared visible-change rule used by stats, diffs, pending/recent overlays, and preview framing. Overlay rendering caps the number of visible sections drawn each frame, uses stricter per-frame upload budgets for recent/pending previews, and prioritizes the nearest sections.
...
- the workspace home screen is Build History: a compact owo-ui window with `Save build` as the only primary action, one-click `See changes`, and recent saves. Recent save cards create branches through an in-place modal backed by `BranchCreationDialogStateFactory` and `BranchCreationDialogView`; successful branch-from-save creation immediately follows with the normal branch switch workflow and selects the new branch in history. Branch history lists show direct branch saves together with the selected branch's shared base/head save, so a newly created branch immediately displays the save it was created from and can restore that save against the new branch. Restore confirmations also use modal overlays instead of inserting confirmation panels into the content flow. The recent-save branch picker only changes which branch saves are displayed; explicit switching between existing branches stays on the Branches screen through `VariantService`.
...
- Import / Export and Settings are first-level workspace sidebar routes, while `More` keeps storage cleanup, manual highlight, the interactive history graph, raw references, and the bottom manual update check in one place
...
- partial restore is exposed from Save details `More`. The form accepts either the current Lumi selection or manually edited bounds, supports `Only selected area` and `Everything except selection`, then requires a preview before apply. Successful partial restores apply the target save state into the world, replace the affected pending draft state without moving the branch head or creating a save, mark the persisted pending draft as current-run work, and record the runtime undo/redo stack as one action.
- Save details shares the same branch-creation dialog as Build History for creating a branch from the selected save. The Branches tab remains the place for branch switching, merge, and inactive-branch deletion.

## History architecture

Current runtime history behavior:

- `HistoryCaptureManager` still records explicit tracked block changes inside project bounds, including TNT ignition, explosions, and selected mob block mutations, while still excluding Lumi's own restore applications. `CaptureEligibilityService` owns source/access, causal-action, direct/deferred, and hidden builder-surface gates. Durable working-draft buffers, session states, dirty flags, recovery draft persistence, and live-draft flush fingerprints are owned through `WorkingDraftSessionManager`; project metadata loading and membership matching are owned by `TrackedProjectCatalog`; session baseline snapshots and baseline corrections are owned by `CaptureBaselineCoordinator`; accepted-mutation diagnostics and skipped-capture throttling are owned by `CaptureDiagnosticsLogger`, so the mixin-facing facade does not directly manage every lifecycle, catalog, or diagnostics map.
- Primed TNT keeps the builder action context that created it until the delayed `Level.explode`/`ServerLevel.explode` call, so the later block damage is recorded into the same draft and live undo action instead of being treated as unrelated ambient explosion noise. Primed entities from direct ignition or redstone block-update activation are live undo-only: undoing before the fuse completes removes the entity and restores the TNT block without persisting that transient entity into saves. Internal restore/undo/redo replay suppresses TNT `onPlace` and neighbor activation callbacks while capture is suppressed, and the exact replay guard briefly protects restored TNT positions, so TNT restored next to redstone does not immediately re-prime itself.
...
- Player-rooted item entity edits are normal durable entity changes. Item entities produced by explosion, fluid, falling-block, and nearby block-update fallout, plus primed TNT entities produced by ignition or redstone activation, are captured as undo-only related entities. They are deliberately excluded from recovery drafts and version payloads, so correcting an edit clears the dropped items or active primed TNT without turning transient entities into durable project history. Entity snapshots keep the full saved entity NBT, including tick fields such as item age, motion, pickup delay, and fuse state, so respawn/update replay does not partially reset entity state.
...
- New live capture sessions are also limited to explicit builder-driven sources. Whole-dimension sessions seed a causal chunk envelope from those root edits, then keep secondary capture bounded to that active region or chunks currently loaded for a player only while the secondary source still carries or can reuse causal action ownership. Deferred block stabilization still requires the originating action id, while nearby mechanism entity fallout can join the latest matching live undo action without bootstrapping durable capture by itself. Per-chunk session baselines are captured lazily as compact chunk snapshot payloads only when a chunk inside that active session region first needs stabilization.
...
- For whole-dimension workspaces, action-scoped fluid spread and falling blocks no longer append directly into the draft. They only re-mark chunks inside the active session region as dirty while carrying or reusing causal action ownership, and `SessionStabilizationService` later rebuilds the final chunk diff by comparing compact chunk snapshots instead of walking the world through `level.getBlockState()`. Captured and deferred mutations retain per-position baseline corrections from their pre-change state, including deferred mechanism cells in the same root chunk as the input action, and secondary deferred sources can capture a clean session chunk baseline before their first block write when an active session already owns the region and the source still has an action owner. Direct same-session block captures are rebased through `SessionBaselineStateResolver` before they enter the recovery draft, so a transient event-local old state does not replace the session baseline; live undo/redo still records the original event transition. Delayed fluid and falling-block ticks can reuse the pending dirty chunk action context after their scheduled callback loses the current stack frame; if no action context remains, the callback is treated as passive world settling and skipped. This keeps redstone, redstone power blocks, target, dispenser, water-break, powered TNT, and piston fallout from turning lazy baseline snapshots into the apparent original state without letting underground fluids loaded by flying become pending changes. Replay restores derived redstone property states exactly; it notifies neighbors for signal-source block changes and for restored player input signal changes, and a short bounded exact-state guard suppresses stale replay callbacks for derived volatile states and restored TNT while excluding player controls plus active piston/observer mechanism participants and yielding to new explicit edits. Fluid-related replay placements also get a short post-replay fluid suppression envelope so scheduled water/lava ticks queued before undo/redo/restore cannot immediately refill blocks that history just restored; recorded-only fluid guards run even when no final exact-state drain is required, and replay-to-air targets immediately clean loaded non-source target fluid plus bounded orphaned non-source tail components cut off from source/saved fluid by those restored dry targets. Source-connected older/generated water and saved source water/lava are not deleted. Live undo/redo asks that same stabilization path to drain currently dirty chunks before it selects the next action; if a dirty diff lands on a block already tracked by an explicit builder edit, the live undo action records only the transition from that explicit current state to the settled fallout state, not from the chunk baseline.
- Save, amend, recovery, restore, branch-switch, and undo/redo completion paths that need the working draft marshal snapshot/freeze/consume/discard/adjust work onto the Minecraft server thread before touching loaded chunks or mutable capture state. Save/amend queue their world operation before consuming the live draft or writing `operation-draft.bin.lz4`, so large draft isolation is reported as operation progress instead of blocking the initiating screen. Save/amend still consume the live working draft before reading any persisted fallback, so active async WAL flushes are drained before recovery storage is inspected; project bootstrap also skips interrupted operation-draft promotion while a Lumi world operation is active, preventing a current save/amend draft from being reclassified as recovery before the operation finishes. Active save/amend failure paths leave `operation-draft.bin.lz4` isolated for later idle recovery instead of promoting it into the live draft. Save/amend consume only the working draft and leave the volatile undo/redo stack intact; if a new draft appears while the async save runs, the draft base is rebased from the consumed head to the newly saved version. Full and partial restore queue their operation before lineage loads, capture freeze, journal writes, and decode planning. Branch creation only writes metadata for an existing save/head and intentionally does not freeze or consume the active draft. The Build History and Save details branch-from-save actions compose metadata creation with branch switching, so the switch path still owns draft validation and restore preparation. Branch switching restores through an explicit target branch so a branch created from a main-line save stays active after the restore operation completes.
...
- Secondary explosion, fire, block-update, falling-block, fluid, and mob sources are gated by the active session region, so one explicit edit does not pull unrelated cave settling or water into a new draft. Direct explosion, fire, growth, fluid, falling-block, and mob capture require a causal action id, and deferred redstone/piston/fluid/falling-block stabilization also requires one. Redstone, piston, fluid, and falling-block callbacks can fall back to the latest pending dirty-chunk action context for late callbacks, but fluid and falling-block stabilization no longer proceeds actionlessly just because the chunk is player-loaded. Related live undo/redo entity fallout is bounded by the nearby action join policy. That region includes the causal envelope and chunks currently loaded for a player.
...
- Preview generation now queues lightweight request files on the server side and fulfills them later through the client-side `PreviewCaptureCoordinator`, which backs off after empty scans to avoid idle storage polling. UI preview textures are invalidated when the backing PNG timestamp or size changes, kept in an approximate byte-budgeted cache, and released when the client leaves a world.
- `RestoreService` prefers direct linear patch replay for ancestor/descendant restores and `WORLD_ROOT` ancestor restores, exposes a lightweight restore plan summary for `Initial` confirmation that is based on the target snapshot chunks, includes pending recovery-draft chunks in that summary even when the selected save is already the active head, appends exact `INITIAL` snapshot state only for changed block positions produced by pending-draft rollback or direct patch replay, appends exact `WORLD_ROOT` baseline state only for changed positions in touched chunks with required tracked baselines, carries that exact-root marker through apply progress for runtime-budget verification, fails planning when required root baseline data is missing, falls back to tracked baseline chunks or checkpoint snapshot plus patch chain when direct replay is not valid, and asks `RestoreCompletionCoordinator` to reset the active branch head to the restored save after apply without deleting detached saves. Direct restore decode also receives `MechanismReplayScope`; redstone/mechanism target-state reconciliation is resolved by exact position off-thread, `INITIAL`/`WORLD_ROOT` may expand bounded touched mechanism sections, and scopes above 65,536 extra cells force the existing snapshot/patch-chain fallback. `RestoreRequestResolver` owns target version/branch resolution and imported-package trust gating before restore planning, `RestorePlanBuilder` owns the checkpoint snapshot, patch metadata chain, and whole-dimension baseline-gap plan, `RestorePayloadLoader` owns version patch metadata lookup plus full/selective block and entity payload reads, `BlockTargetStateResolver` owns exact-position block target-state reconstruction from snapshots, baselines, and patches, and `RestoreEntityStateResolver` owns entity target-state reconstruction plus authoritative entity replacement batches. Divergent branch-head restores use the target snapshot/patch-chain path instead of sparse reverse/forward replay so derived redstone and fluid fallout is reset to the target branch state. Exact root snapshot/baseline reads stay sparse unless bounded mechanism reconciliation applies, and entity target-state reads use patch entity-id and old/new chunk indexes when available, so direct root and boundary-crossing entity restores do not scan full snapshots or full patch payloads on the common path. Full restores write `recovery/last-restore-return.json` so quick return can restore the pre-restore version; dirty drafts are checkpointed first so the return target is exact. Full restore and full quick rollback append authoritative entity replacement batches for the affected chunks so extra non-player entities, including restored/display entities and transient item drops, are removed and duplicate UUID spawns become idempotent. Full restore completion clears stale live undo/redo stacks because old builder actions no longer describe the restored world, and pending restore-completion records let idle recovery finish metadata publication if the completion callback was interrupted after successful apply. `QuickRollbackService` freezes the current dirty draft, verifies it belongs to the active head, applies the inverse draft through the `quick-rollback` history-fast prepared apply path, resolves mechanism halo target states for full rollback only inside project/world bounds, clips that halo again to the selection for selected rollback, discards the rolled-back draft portion, and records a fresh live undo/redo action so normal undo/redo can toggle the rollback without moving the saved branch head. Explicit return-before-restore remains a UI-triggered hard restore path, not a keyboard chord. Restores from a save on another branch plan from the current live branch and change active-branch metadata only after apply completion. Persisted block/entity changes and snapshot entity payloads are read by repositories, prepared by Minecraft-layer batch preparers, completed for paired blocks such as beds, doors, tall plants, and settled piston base/head companions, and then applied through the operation model; repositories do not assemble tick-runtime batches.
- `RestoreService` also supports same-lineage selected-area restore from save details, including a branch restoring a bounded area from the save it was branched from. It filters pending draft and direct patch block/entity changes to manual bounds, reads only intersecting chunk-addressable patch frames when possible, uses old/new entity chunk metadata to include move-out and move-in records, applies prepared batches through the operation model, then delegates pending-draft replacement and undo/redo recording to `RestoreCompletionCoordinator`.
- `VariantService` keeps one head pointer per branch.
...
- `VariantMergeService` turns imported review projects and local branches back into local history by finding a shared saved ancestor, projecting incoming block and entity changes over the current branch state, and delegating merged version persistence to `VersionService` with `VersionKind.MERGE`. When both branches changed the same block or entity, the incoming branch state wins without asking the player to resolve the overlap. Start requests stay caller-thread-light; merge planning, safety scanning, incoming-over-current projection, and block/entity batch preparation execute inside the world-operation executor.
...
- bounded chunk preload and chunk-batch application on the server tick through `WorldOperationManager`, including adaptive block budgets, explicit preload ticket caps, no default synchronous chunk-load budget during preload, stall-aware one-at-a-time sync recovery only after all ticketed chunks stop making progress, `WorldApplyPerformanceGovernor` decisions from `ApplyCostModel`, fast-profile-only synchronous chunk reacquire after preload misses, fast-profile mixed rewrite/native/sparse tick work, profile-specific minimum sparse/direct time floors after adaptive downscale, and explicit block-entity/entity caps
...
- internal restore/undo/redo/recovery apply suppresses incidental `ItemEntity` drops from vanilla block side effects, while deliberate saved entity spawns run inside an explicit entity-replay frame and still enter the world normally
...
- fast prepared apply operations expose a `PRELOADING` stage before mutation; it uses temporary chunk tickets, releases them on completion or failure, waits for ticketed chunks instead of synchronously loading them by default, and switches to one sync preload fallback per tick only after all chunks are ticketed and ticket progress has stalled. It records preload duration, ticks, loaded-before-apply chunks, newly loaded chunks, preload sync fallback counts, missed-at-apply fallback counts, and max preload tick time. If preload completes within the current server tick, apply may begin in that same tick instead of waiting for the next tick. After preload, only fast profiled apply ticks may synchronously reacquire a chunk that was part of the operation when `getChunkNow` still misses, and direct, native, and rewrite paths share that contract. Loaded history-fast chunks are also pruned once before apply so cells that already match live state do not consume tick-time replay budget; no-op exact replay is retained only for forced replay companions, cells adjacent to real updates, and chunk-edge cells that may depend on cross-chunk neighbors. If a loaded live cell currently contains fluid and the prepared target is dry, no-op pruning promotes the placement to the same fluid replay cleanup hint used by recorded fluid targets, so restoring a block beside pre-existing water can immediately clear fluid from that action's restored target and orphaned downstream non-source tails without enabling broad placement physics or erasing source-connected older water. Lightweight undo/redo completion runs on the server tick after replay so stack movement and pending-draft adjustment do not need a background-thread round trip.
- optional debug tracing for capture, save, restore, recovery, compare, HUD, and background operations
- opt-in stack-trace fallback detection for unsupported external builder tools with `-Dlumi.externalStackDetection=true`; normal development and test-client runs should rely on explicit integrations instead of sampling Java stacks on every vanilla mutation
...
The client checks for Lumi releases at startup and refreshes the same check when the Build History project screen opens. Startup checks ignore the 12-hour freshness cache so edited GitHub release metadata and manifest summaries can replace stale local prompt text without asking testers to delete config. The check is client-only, runs off the render thread, and only prompts for a stable or alpha Fabric release whose `minecraftVersions` contains the installed Minecraft version and whose mod version is newer than the installed Lumi version. A promptable release is surfaced once per game session in world chat as an aqua `New Lumi <version> is available!` line with `Lumi` bolded, a gray changelog header/body, `•` bullets for list-style changelog lines and plain lines after a summary heading ending in `:`, a red GitHub issues line, and a clickable `[Download] (click)` component that opens the GitHub release page. Build History renders the same release as a modal with `Skip` and `Download` actions over the current screen, and More exposes a bottom `Check updates` action that opens either that update modal or an up-to-date/failure info modal.
...
- point `downloadUrl` and `changelogUrl` at the exact GitHub release tag page, for example `https://github.com/KristopherZlo/lumi/releases/tag/v0.1.0-alpha.2`, so the player can review release notes before downloading. Use direct release asset URLs only for explicit one-click download experiments.

To test an update through GitHub, build a higher local version such as `0.1.0-alpha.2`, create and push tag `v0.1.0-alpha.2`, publish a GitHub release with the jar attached, then commit `updates/lumi-fabric.json` with that same version, a higher `versionCode`, `channel` set to `alpha`, `minecraftVersions` containing `1.21.11`, multi-line `summary` change notes, and URLs pointing at the test release tag. Launch the test client, enter a world, confirm the chat notice shows the styled release line, gray changelog with preserved line breaks and `•` bullets for explicit list markers or plain lines under a `:` heading, a clickable `[Github]` issues link, and a clickable `[Download] (click)` release-page link. Then open Build History and verify the update modal's `Skip` and `Download` actions, and open More -> `Check updates` to verify both the update modal and, after testing against a current manifest, the latest-compatible-version info modal.
