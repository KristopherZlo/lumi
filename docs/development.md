...
- redstone and mechanism state is durable final block state: lever `powered`, wire `power`, lamp `lit`, openable `open`, repeater/comparator properties, piston base `extended`, settled `piston_head`, and moved blocks are saved. The apply preparers complete settled piston base/head companions before replay, clear an old head when a known extended base retracts, and deliberately never infer a new piston base from a head-only placement; only short-lived `moving_piston` animation state is normalized away
