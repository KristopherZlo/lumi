...
- pressing the Lumi action button plus `Y` starts redo for the latest tracked action in the current dimension workspace while no client screen is open; if undo and redo are pressed in the same tick, undo wins and redo must be pressed again. WorldEdit and FAWE actors route through native `/undo` and `/redo`, with Lumi capture suppressed during the command and the pending draft adjusted afterward. Axiom actors and `axiom-*` action ids replay through Lumi by default so Lumi applies the same action it selected from its own history stack.
...
- runtime-only redstone state flips and piston animation states are routed through deferred stabilization and then filtered if they only change runtime properties, so they do not become live undo/redo actions or durable patch entries
