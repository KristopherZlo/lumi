...
- Lumi-owned cursor-sliced section-native commits for dense prepared sections, direct loaded-section commits for sparse batches, with vanilla fallback and batched section/client block-entity updates. Those Lumi-owned paths defer light checks until after the block writes in the current section batch or completed native cursor. Sections with at least 64 prepared cells are considered dense enough for the native loop path, and full sections or sections with at least 1024 prepared cells may use the atomic container rewrite path when preflight proves there are no block entities or POI states. Full-section rewrite builds a fresh replacement container from all target cells, while partial dense rewrite still copies the live container before patching changed cells.
