...
- the workspace home screen is Build History: a compact owo-ui window with `Save build` as the only primary action, one-click `See changes`, and recent saves. Recent save cards create branches through an in-place modal backed by `BranchCreationDialogStateFactory` and `BranchCreationDialogView`; restore confirmations also use modal overlays instead of inserting confirmation panels into the content flow. The recent-save branch picker only changes which branch saves are displayed; real branch switching stays on the Branches screen through `VariantService`.
