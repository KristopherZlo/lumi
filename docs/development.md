...
- undo/redo drains already-dirty stabilization chunks before selecting an action, so reconciled fluid, contact-created source blocks, falling-block deltas, redstone block updates, and piston fallout can join the latest nearby undo/redo action when they settle inside the same time/radius window without discarding redo that came from the last undo
...
- debug apply metrics include rewrite sections/cells, native sections/cells, deferred light checks, section packets, and rewrite/native fallback reasons so slow dense actions can be traced to safety rejection, light maintenance, block entities, unloaded chunks, or sparse distribution.
