...
`LdLib2InterfaceBlueprint` is the current migration contract for the home screen. It maps the simple builder flow to LDLib2 element roles (`UIElement`, `Label`, `Button`, `ScrollerView`, and `TabView`), pins the built-in GDP theme (`ldlib2:lss/gdp.lss`), and records compact flex hints for the future LDLib2 tree while the runtime screen still renders through the compatible fallback. `ProjectWindowLayout` mirrors that contract visually with a compact bordered app window, sidebar, title bar, status strip, and scrollable body.
