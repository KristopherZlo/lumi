...
- Fresh worlds created while Lumi is installed write `<world>/lumi/created-with-lumi.marker` before server-side world-origin bootstrap. Existing pre-Lumi worlds without a completed pre-mod backup show a client alpha backup gate before opening, create the backup before world entry with visible Minecraft experience-bar progress, and store the acknowledgement at `<world>/lumi/pre-mod-backup/alpha-backup-warning-acknowledged.txt` after the backup succeeds. The vanilla Edit World restore action delegates to `WorldInitialBackupRestoreService`, writes backed-up raw chunks into region files, and keeps Lumi project commits on disk.
