...
- `VariantMergeService` turns imported review projects and local branches back into local history by finding a shared saved ancestor, grouping overlapping conflicts into chunk-connected review zones, carrying non-conflicting entity changes, rejecting unresolved entity conflicts explicitly, and delegating merged version persistence to `VersionService` with `VersionKind.MERGE`. Start requests stay caller-thread-light; merge planning, safety scanning, conflict resolution, and block/entity batch preparation execute inside the world-operation executor.
