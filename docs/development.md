...
- Fresh worlds created while Lumi is installed write `<world>/lumi/created-with-lumi.marker` before server-side world-origin bootstrap. Existing pre-Lumi worlds without a completed pre-mod backup show a one-time client alpha warning before opening, and the acknowledgement is stored at `<world>/lumi/pre-mod-backup/alpha-backup-warning-acknowledged.txt`.
