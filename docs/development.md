...
- comparing against `current` refreshes the active world highlight automatically on the normal client cadence while that highlight is visible, except active overlays above the 50,000-block detailed-render cap which keep their initial snapshot to avoid client stalls. If the refreshed diff is unchanged, the existing overlay meshes are reused.
