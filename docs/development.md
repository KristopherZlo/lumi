...
- redstone and mechanism state is durable final block state: lever `powered`, wire `power`, lamp `lit`, openable `open`, repeater/comparator properties, piston base `extended`, settled `piston_head`, and moved blocks are saved; only short-lived `moving_piston` animation state is normalized away
