...
первого окна и resident-prefix (не более 1024 block chunk и 256 entity chunk) передаются
новому плану. Каждый session освобождает завершённое окно до загрузки следующего;
несовместимый readiness mode закрывает tickets и включает обычную bounded загрузку.
Совпадение промежуточного состояния
