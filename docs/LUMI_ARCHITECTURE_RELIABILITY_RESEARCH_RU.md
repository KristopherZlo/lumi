...
первого окна передаются новому плану, только если совпали scope и readiness mode. Иначе tickets
закрываются и используется обычная bounded загрузка. Совпадение промежуточного состояния
