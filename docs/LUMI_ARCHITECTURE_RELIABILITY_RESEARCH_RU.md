...
| rejected resident direct-write, 2026-08-04 | click→complete 5.674/5.303/5.648 s, median 5.648 s; warm write 1.217/1.547 s | report `20260804-232732-history-benchmark-existing-world-branch-switch`; only 156–162 of 871 chunks used the experimental path, while the remaining chunks still required vanilla Save. The duplicate I/O regressed latency, so the path was reverted |
| restored vanilla-save fallback, 2026-08-04 | click→complete 6.008/4.600/4.093 s, median 4.600 s | report `20260804-234327-history-benchmark-existing-world-branch-switch`; one vanilla persistence path again owns all changed chunks; exact reopen, entity UUID and runtime-health gates passed |
| bounded block-ticket prewarm, 2026-08-05 | click→complete 4.721/3.855/4.109 s, median 4.109 s; median chunk readiness 1.454 s; warm write 669/675 ms, force 43/47 ms, verification 487/509 ms | report `20260805-000206-history-benchmark-existing-world-branch-switch`; prewarm hands off only verified chunk readiness, never stale decoded or live state. Incomplete or incompatible readiness uses the ordinary cold path. Exact reopen, entity UUID and runtime-health gates passed. This is `1.42×` against the 5.853 s baseline and `10.7%` below the restored vanilla-fallback median, not the target `3–4×` |
| rejected entity-ticket prewarm, 2026-08-05 | click→complete 5.156/4.007/4.156 s, median 4.156 s; median chunk readiness 1.457 s; max extra heap 770,590,192 B | report `20260805-000826-history-benchmark-existing-world-branch-switch`; no measured readiness gain over block-only prewarm, with worse prewarm duration and memory, so the change was reverted |
...
неизменившейся части. Stale decoded slab при активации отменяется, но готовые chunk tickets
первого окна и resident-prefix (не более 1024 chunk) передаются новому плану. Один session
освобождает завершённое окно до загрузки следующего; несовместимый readiness mode закрывает
tickets и включает обычную bounded загрузку. Совпадение промежуточного состояния
проверяется до передачи владения.
