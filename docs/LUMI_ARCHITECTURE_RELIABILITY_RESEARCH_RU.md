...
После открытия измерения scheduler один раз пытается выполнить только idle pack compaction через 200 ticks; если измерение занято, попытка откладывается. Полный reachability GC сохраняет исходную задержку 6000 ticks и часовой интервал.
