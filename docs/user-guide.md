...
- Undo and redo restore the stored block states with side-effect-suppressed placement flags. Durable redstone and mechanism state changes, such as undoing a lever or button toggle, send scoped neighbor updates so nearby circuitry settles, while ordinary replay still avoids placement physics and piston event cascades.
