...
- Undo and redo restore the stored block states with side-effect-suppressed placement flags. Redstone power/source changes, such as undoing a lever or button toggle, send scoped neighbor updates after the stored state is written so nearby circuitry settles, while ordinary replay still avoids placement physics and piston event cascades.
