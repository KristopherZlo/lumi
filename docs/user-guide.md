...
- Undo and redo restore the stored block states with side-effect-suppressed placement flags. Redstone power/source changes, such as undoing a lever or button toggle, send scoped neighbor updates after the stored state is written so nearby circuitry settles, while ordinary replay still avoids placement physics and piston event cascades. Delayed redstone callbacks that were queued by the action before undo/redo are dropped after that action is replayed, so a repeater chain cannot keep propagating the old signal. Replay does not hold levers, buttons, pressure plates, or tripwire controls after the operation; a new explicit edit releases the short redstone stabilization guard.
- Oscillating redstone state can settle into the action that created it while that action is still current, but it cannot jump ahead of later player edits in the undo order.
