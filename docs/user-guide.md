...
Secondary effects such as falling gravel, fire spread, fluid spread, redstone-driven block updates, ice melting, and TNT or explosion fallout only join a draft after an explicit tracked action has already started that draft, and only while they stay inside the active session region.
