...
- If a newer Lumi version is available for the same Minecraft version, Lumi writes a clickable download notice in chat after world entry and shows update modals in Build History and More.
...
Redstone power/source changes queue scoped neighbor updates after stored blocks are written so nearby circuitry can settle. Fluid replay queues bounded vanilla fluid ticks around loaded connected fluid tails while exact replay keeps the stored target cells fixed; Alt+Z clears live flowing fluid from the current action's restored cells, but does not erase older water-filled cells from earlier digs or saved source water/lava. Ordinary replay still avoids placement physics and suppresses stale replay callbacks around piston/observer mechanism positions.
...
Lumi writes the selected target state into the world as pending unsaved work. It does not move the branch head back to the older save and does not create a save until you explicitly save. During the same game session, reopening the workspace shows that pending work in Build History instead of forcing `Recovered work`.
...
The bottom `Check updates` action performs an immediate GitHub-backed compatibility check. If a newer compatible Lumi release is found, it opens the update modal; otherwise it confirms that the installed Lumi version is the latest compatible build.
