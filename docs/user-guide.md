...
- Undo and redo restore the stored block states with side-effect-suppressed placement flags. Redstone power/source changes, such as undoing a lever or button toggle, send scoped neighbor updates after the stored state is written so nearby circuitry settles, while ordinary replay still avoids placement physics and piston event cascades. Replay does not hold levers, buttons, pressure plates, or tripwire controls after the operation; a new explicit edit releases the short redstone stabilization guard.
