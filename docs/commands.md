...
`/lumi testing structures` runs only the saved structure fixture diagnostics. It creates the same temporary bounded project, then skips the broader save, restore, gameplay, bulk apply, and performance phases. This keeps the default client GameTest focused on UI, overlay, storage, gameplay, large-history, and bulk-apply coverage while the alpha release wrapper still runs the strict fixture rollback suite as a separate focused mode.
