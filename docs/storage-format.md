...
- settled redstone and mechanism state is stored with the normal block-state NBT already present in patch palettes. Lever `powered`, wire `power`, lamp `lit`, openable `open`, repeater/comparator properties, piston base `extended`, settled `piston_head`, and moved blocks are schema-compatible state deltas; no schema bump is needed because these properties already fit the existing block-state tag payload. Apply preparation may synthesize missing settled piston head/removal companions from a recorded piston base for replay safety, but storage still contains ordinary per-position old/new state tags. Only short-lived `moving_piston` animation state is normalized to air before new patch payloads are written
