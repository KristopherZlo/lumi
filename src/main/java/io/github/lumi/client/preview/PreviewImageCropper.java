...
/** Removes transparent offscreen-render margins while retaining two pixels of air. */
final class PreviewImageCropper {
    private static final int PADDING = 2;

    NativeImage crop(NativeImage source) {
        ...
        for (int y = 0; y < source.getHeight(); y++) {
            for (int x = 0; x < source.getWidth(); x++) {
                if (alpha(pixels[index(source.getWidth(), x, y)]) == 0) continue;
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                cropped.setPixelABGR(
                        x, y, pixels[index(source.getWidth(), cropMinX + x, cropMinY + y)]);
            }
        }
        source.close();
        return cropped;
